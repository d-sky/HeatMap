...
namespace HeatMap
{
    ...
    public class HeatMap
    {
        ...
        double GetPointValue(int limit, PointWithValue point, List<PointWithDistance> pointsWithDistance)
        {
            long distance = 0;
            double inv = 0.0,
                    t = 0.0,
        ...
        public Bitmap Draw()
        {
            ...
            while (y < yEnd)
            {
                var val = GetPointValue(pointsLimit, new PointWithValue() { X = x, Y = y }, pointsWithDistance);
